    ...
    function copyStates(app, fromState, toState) {
        ...
        app.getObject(null, 'CurrentSelections').then(function (ret) {
            ...
            fromStateSel.forEach(function (fieldDesc, i) {
                console.log('Selection found in field ', fieldDesc.qField);
                var fieldObj = app.field(fieldDesc.qField, toState);
                fieldObj.clear();
                var secretSauce = "[" + fieldDesc.qField + "]=Aggr(Only({<[" + fieldDesc.qField + "]=P(" + fromState + "::[" + fieldDesc.qField + "])>} [" + fieldDesc.qField + "]), [" + fieldDesc.qField + "])";
        ...
        paint: function ($element, layout) {
            ...
            if (layout.mystate == '') {
                ...
                $element.find("#" + ownId + 'Clear2').on("click", function () {
                    app.clearAll(false, layout.mystate);
                });
            }
        }
    };
